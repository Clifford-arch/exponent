...
const Indian = () => {
  ...
  const [graphData, setGraphData] = useState<number[]>(
    sensexGraphData[selectedGraph.sensex]["1D"]
  );
  ...
  const renderPeriodButton = ({ item }: { item: Period }) => (
    <TouchableOpacity
      ...
      style={[
        styles.periodButton,
        activePeriod === item && styles.activePeriod,
      ]}
    >
      <Text
        style={[
          styles.periodText,
          activePeriod === item && styles.activePeriodText,
        ]}
      >
        {item}
      </Text>
  ...
  const renderIndicesItem = ({
    item,
  }: {
    item: (typeof sampleIndicesData)[0];
  }) => (
    <TouchableOpacity
      onPress={() => handleGraphClick(item)}
      style={[
        styles.indicesItem,
        selectedGraph.id === item.id && styles.selectedIndicesItem,
      ]}
    >
          ...
          <View style={styles.statsContainer}>
            ...
            <View style={styles.previousCloseContainer}>
              {Object.entries(graphDataIndices.PreviousClosePerc)
                ...
                .map(([key, value]) => {
                  const dirKey =
                    `${key}_dir` as keyof typeof graphDataIndices.PreviousClosePerc;
